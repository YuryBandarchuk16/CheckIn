...
class GameScene: SKScene {
    ...
    func addNewCookies(location: Array<Cookie>) {
        print("-------")
        level.printCookies()
        for c in location {
            let cookieType = CookieType.random()
            let sprite = SKSpriteNode(imageNamed: cookieType.spriteName)
            sprite.size = CGSize(width: TileWidth, height: TileHeight)
            sprite.position = pointFor(column: c.column, row: c.row)
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + wait, execute: {
                ...
                c.sprite = sprite
                c.cookieType = cookieType
                self.level.cookies[c.column, c.row] = c
    ...
    // animate the swapping of the cookie
